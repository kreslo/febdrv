...
void free_subbufer (void *data, void *hint) //call back from ZMQ sent function, hint points to subbufer index
{
 //return;
 int sbi;
 sbi =  (int)hint; //reset buffer status to empty
 evbufstat[sbi]=0;
 evnum[sbi]=0; 
...
//        else { ts1=tt1; ts1_ref=ts1_ref_MEM[rpkt.src_mac[5]]; }
        ...
        if((ts0 & 0x80000000)>0) {REFEVTts0=1; ts0=tt0; ts0_ref=tt0; ts0_ref_MEM[rpkt.src_mac[5]]=tt0;} 
        else {REFEVTts0=0; ts0=tt0; ts0_ref=ts0_ref_MEM[rpkt.src_mac[5]]; }
        if((ts1 & 0x80000000)>0) {REFEVTts1=1; ts1=tt1; ts1_ref=tt1; ts1_ref_MEM[rpkt.src_mac[5]]=tt1;} 
        else {REFEVTts1=0; ts1=tt1; ts1_ref=ts1_ref_MEM[rpkt.src_mac[5]]; }
